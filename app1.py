...
if page == "Prediction":
    ...
    if run:
        ...
        # Probability metrics
        m1, m2 = st.columns(2)
        m1.metric("Alive probability", f"{alive_pct}%" if alive_pct is not None else "N/A")
        m2.metric("Dead probability",  f"{dead_pct}%"  if dead_pct  is not None else "N/A")

        if alive_pct is None:
            st.info("ℹ️ SVM was trained without `probability=True`, so confidence scores are unavailable. Only the predicted class is shown.")
...
# ── COMPARE MODELS PAGE ────────────────────────────────────────────────────────
    ...
    if run_all:
        ...
        for name in MODEL_NAMES:
            ...
            has_proba_m = hasattr(m, "predict_proba") and (
                getattr(m, "probability", True) is not False
            )
            alive_pct_c = round(m.predict_proba(scaled)[0][0] * 100, 1) if has_proba_m else None
            results.append({
                "model":      name,
                "prediction": "Alive" if pred == 0 else "Dead",
                "alive_pct":  alive_pct_c,
            })
        ...
        for r in results:
            ...
            if r["alive_pct"] is not None:
                ...
                bar_html = '<span style="font-size:12px;color:#aaa;">N/A (no proba)</span>'
            table_html += (
